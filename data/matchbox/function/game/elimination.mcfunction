...
execute as @s[scores={deaths=1..},tag=!out] run title @s title {"text":"You have been eliminated!","color":"red"}
execute as @s[scores={deaths=1..},tag=!out] run title @a[tag=!out,scores={deaths=0}] title [{"selector":"@a[tag=!out,gamemode=spectator]","color":"red"},{"text":" was Eliminated!","color":"dark_red"}]
