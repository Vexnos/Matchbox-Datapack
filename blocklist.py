with open("blocks.txt") as file:
  lines = [line for line in file.readlines() if len(line.split("|")) > 2]

blocks = []

for line in lines:
  _, name, added, removed = line.split("||")
  if removed.strip() == "-" or "," in added:
    blocks.append(name)
...
with open("data/matchbox/function/game/items.mcfunction", "w") as file:
  file.write(command)
